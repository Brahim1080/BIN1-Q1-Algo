...
public class TriParInsertion {
	...
	public static int[] tri (int[] t) {
		//insertion
		//recopie le tableau dans un autre tableau
		...
		for (int i = 1; i < tabletrie.length; i++) {
			elemActuel = tabletrie[i];
			j = i - 1;
			while (j >= 0 && elemActuel < tabletrie[j]) {

				temp = tabletrie[j];
				tabletrie[j] = tabletrie[j + 1];
				tabletrie[j + 1] = temp;
				j--;
			}

		}
		// La table passee en parametre ne peut pas etre modifiee
