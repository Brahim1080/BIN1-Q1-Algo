...
public class Vol {
	...
	public Coordonnees niemeLieu(int n){
		...
		if(tableCoordonnees.length >= 0&&n>tableCoordonnees.length) {
	...
	public Coordonnees lieuLePlusEloigne(){
		...
		for (int i = 1; i < tableCoordonnees.length; i++) {
			if(tableCoordonnees[i].distance(tableCoordonnees[0])>tableCoordonnees[0].distance(max))
				max = tableCoordonnees [i];
	...
	public int nombreCiblesAtteintes(Coordonnees[] cibles){
		...
		for (int i = 0 ; i < cibles.length ; i++){
			if (cibleAtteinte(cibles[i]) == true){
				nmbrCibles++;
			}
		}
